...
encounter <- function(allInd){
  patchMales <- patchHere(world = w1, 
                          turtles = NLwith(agents = allInd, 
                                           var = "sex", 
                                           val = "male"))
  patchFemales <- patchHere(world = w1, 
                            turtles = NLwith(agents = allInd, 
                                             var = "sex", 
                                             val = "female"))
  patchFemalesWho <- cbind(patchFemales, 
                           whoReproducingFemales = of(agents = NLwith(agents = allInd, 
                                                                      var = "sex", 
                                                                      val = "female"), 
                                                      var = "who"))
  
  # Patches in both group
  patchBoth <- merge(as.data.frame(patchMales), 
                     as.data.frame(patchFemalesWho))
